...
public class Population extends Configured implements Tool {
	...
	public static class PopulationReducer extends
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			while (values.iterator().hasNext()) {
				long currentValue = Long.parseLong(values.iterator().next()
						.toString());
				sum += currentValue;
			}
			population.set(sum);
	...
	public int run(String[] arg0) throws Exception {
		...
		URI partitionerUri = new URI(partitionFIle + "#"
				+ TotalOrderPartitioner.DEFAULT_PATH);
		job.addCacheFile(partitionerUri);
	...
	public static void main(String[] args) {
		...
		try {
			result = ToolRunner
					.run(new Configuration(), new Population(), args);
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.exit(result);
